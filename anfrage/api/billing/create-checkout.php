...
/* ── Config ── */
$secrets = [];
$secretsFile = dirname(__DIR__) . '/_secrets.php';
if (is_readable($secretsFile)) {
  $tmp = require $secretsFile;
  if (is_array($tmp)) $secrets = $tmp;
}

$STRIPE_SECRET_KEY = (string)($secrets['STRIPE_SECRET_KEY'] ?? '');
$STRIPE_PRICE_ID   = (string)($secrets['STRIPE_PRICE_ID'] ?? '');
$SERVICE_ROLE      = (string)($secrets['SUPABASE_SERVICE_ROLE'] ?? '');
$SUPABASE_URL      = 'https://uledkuegmaritmsjejlm.supabase.co';
...
if ($token !== '') {
  ...
  $ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVsZWRrdWVnbWFyaXRtc2plamxtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE1ODk5MjksImV4cCI6MjA4NzE2NTkyOX0.ieblf3gbOOuZiqN_tF6S4dyh-GOgWDtIgdtFbrM5VVU';
