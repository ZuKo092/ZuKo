(() => {
  ...
  const UI = {
    ...
    btnEditSaveBottom: $("btnEditSaveBottom"), btnEditCloseBottom: $("btnEditCloseBottom"),
...
const photoReminderKey="ab_photo_reminder_"+leadId; const lastPhotoReminder=localStorage.getItem(photoReminderKey); if (lastPhotoReminder) { const sentDate=new Date(Number(lastPhotoReminder)); html+=`<div class="hint" style="margin-bottom:6px;font-size:11px;">Erinnerung gesendet: ${sentDate.toLocaleString("de-DE")}</div>`; }
 ...
 async function sendPhotoReminder(leadId) {
  ...
  async function saveSettings() {
    ...
    put("company_name",raw.company_name); put("phone",raw.phone); put("email",raw.email); put("country",raw.country); put("street",raw.street); put("cityline",raw.cityline); put("tax_number",raw.tax_number); put("vat_id",raw.vat_id); put("iban",raw.iban); put("bic",raw.bic); put("footer",raw.footer);
  ...
  function renderList() {
    ...
    for (const r of STATE.filtered) {
      ...
      tr.innerHTML=`<td><div class="tName">${escapeHtml(name)}</div><div class="tSub">${escapeHtml(phone||"")}${email?" • "+escapeHtml(email):""}</div></td>${cols>=6?`<td><div style="font-weight:1000;">${escapeHtml(rt||"-")}</div></td>`:""}<td>${escapeHtml(mdText)}</td><td><span class="pillMini pillStatus">${escapeHtml(st)}</span></td><td class="hint">${escapeHtml(created)}<td><div class="kContact"><button class="btn soft" data-act="open" type="button">Öffnen</button><button class="btn danger" data-act="delete" type="button" style="font-size:11px;">Löschen</button></div></td>`;
      tr.addEventListener("click",(e)=>{ const actBtn=e.target?.closest?.("[data-act]"); const act=actBtn?String(actBtn.dataset.act||""):""; STATE.selectedId=r.id; persistSelected(r.id); renderList(); if (act==="open") { e.preventDefault(); e.stopPropagation(); renderDrawer(); openDrawer(); return; } if (act==="delete") { e.preventDefault(); e.stopPropagation(); if(!confirm("Lead \""+customerName(r)+"\" wirklich löschen?"))return; STATE.sb.from("leads").delete().eq("id",r.id).then(({error})=>{if(error){showNotice("Löschen fehlgeschlagen.","err");}else{showNotice("Lead gelöscht.","ok");STATE.rows=STATE.rows.filter(x=>x.id!==r.id);applyFilters();renderList();renderKPIs();}}); return; } if (act==="pdf") { e.preventDefault(); e.stopPropagation(); openPdfForLead(r.id); return; } if (act==="rechnung") { e.preventDefault(); e.stopPropagation(); if (window.__AB_Rechnung&&typeof window.__AB_Rechnung.create==="function") { window.__AB_Rechnung.create(r.id).then(inv=>{ if (inv&&window.__AB_Rechnung.openPdf) window.__AB_Rechnung.openPdf(inv); }); } return; } if (act==="edit") { e.preventDefault(); e.stopPropagation(); STATE.selectedId=r.id; persistSelected(r.id); openEditModalForSelected(); return; } if (act==="contact") { e.preventDefault(); e.stopPropagation(); openContactModal(r); return; } renderDrawer(); openDrawer(); });
      frag.appendChild(tr);
    }
  ...
  function drawerDetailHTML(row) {
    ...
    const flowSteps=[
      {label:"Anfrage",done:true},
      {label:"Fotos",done:false,id:"flowFotos"},
      {label:"Angebot",done:st==="Angebot gesendet"||st==="Gebucht"||st==="Abgeschlossen"},
      {label:"Angenommen",done:st==="Gebucht"||st==="Abgeschlossen",id:"flowSigned"}
    ];
    const flowHtml=`<div style="display:flex;align-items:center;gap:0;margin-bottom:16px;padding:14px 16px;background:#f8f9fb;border:1px solid #e5e7eb;border-radius:12px;">${flowSteps.map((s,i)=>{
      const active=s.done;
      const dot=`<div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:0 0 auto;"><div${s.id?' id="'+s.id+'"':''} style="width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;${active?'background:#047857;color:#fff;':'background:#e5e7eb;color:#9ca3af;'}">${active?'✓':String(i+1)}</div><div style="font-size:10px;font-weight:700;${active?'color:#047857;':'color:#9ca3af;'}white-space:nowrap;">${escapeHtml(s.label)}</div></div>`;
      const line=i<flowSteps.length-1?`<div style="flex:1;height:2px;${active&&flowSteps[i+1].done?'background:#047857;':'background:#e5e7eb;'}margin:0 6px;margin-bottom:18px;"></div>`:'';
      return dot+line;
    }).join("")}</div>`;

    const hero=flowHtml+`<div class="routeTitle">Route</div><div class="routeRow"><div class="routeBox">${escapeHtml(addrLine(fromA)||"Beladestelle fehlt")}</div><div class="routeArrow">→</div><div class="routeBox">${escapeHtml(addrLine(toA)||"Entladestelle fehlt")}</div></div>`;
  ...
  async function loadPortalCard(leadId) {
    ...
    if (!statusEl||!STATE.sb||!STATE.companyId) { if(statusEl)statusEl.innerHTML='<span class="hint">Nicht verbunden.</span>'; return; }
    try {
    const {data:tk,error:tkErr}=await STATE.sb.from("portal_tokens").select("token,created_at,is_active").eq("lead_id",leadId).eq("company_id",STATE.companyId).maybeSingle();
    if (tkErr||!tk) { statusEl.innerHTML='<span class="hint">Kein Portal-Link vorhanden.</span>'; return; }
    const portalUrl=`${window.location.origin}/portal/${tk.token}`;
    const {data:sig}=await STATE.sb.from("portal_signatures").select("id,created_at,signer_name").eq("lead_id",leadId).maybeSingle();
    const {data:uploads}=await STATE.sb.from("portal_uploads").select("id,file_name,file_url,file_type").eq("lead_id",leadId);
    const photoCount=uploads?uploads.filter(u=>u.file_type&&u.file_type.startsWith("image/")).length:0;
    const signedPdf=uploads?uploads.find(u=>u.file_type==="application/pdf"&&u.file_name&&u.file_name.includes("Angebot")):null;
    ...
    statusEl.innerHTML=`<div style="display:flex;align-items:center;gap:6px;"><span style="width:8px;height:8px;border-radius:50%;background:${statusColor};"></span><span style="font-weight:700;color:${statusColor};">${escapeHtml(statusText)}</span></div>${sig?`<div style="font-size:11px;color:#6b7280;margin-top:4px;">Signiert von ${escapeHtml(sig.signer_name||"Kunde")} am ${escapeHtml(formatDate(sig.created_at))}</div>`:""}`;
    ...
    let btns='';
    if (sig&&signedPdf) {
      btns+=`<div style="margin-bottom:8px;"><a class="btn green" href="${escapeHtml(signedPdf.file_url)}" target="_blank" rel="noopener" style="width:100%;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:6px;font-size:12px;min-height:36px;font-weight:800;">Signiertes Angebot (PDF) →</a></div>`;
    } else if (sig&&!signedPdf) {
      btns+=`<div style="margin-bottom:8px;"><button class="btn green" type="button" style="width:100%;font-size:12px;min-height:36px;font-weight:800;opacity:.5;cursor:default;" disabled>Angenommen (kein PDF)</button></div>`;
    }
    if (!sig&&photoCount===0) {
    ...
    const flowFotos=$("flowFotos");
    if (flowFotos&&photoCount>0) { flowFotos.style.background="#047857"; flowFotos.style.color="#fff"; flowFotos.textContent="✓"; const lbl=flowFotos.parentElement?.querySelector("div:last-child"); if(lbl){lbl.style.color="#047857";} }
    const flowSigned=$("flowSigned");
    if (flowSigned&&sig) { flowSigned.style.background="#047857"; flowSigned.style.color="#fff"; flowSigned.textContent="✓"; const lbl=flowSigned.parentElement?.querySelector("div:last-child"); if(lbl){lbl.style.color="#047857";} }
    } catch(err) { console.error("loadPortalCard error:",err); if(statusEl)statusEl.innerHTML='<span class="hint">Fehler beim Laden.</span>'; }
  }
  ...
  function openEditModalForSelected() {
    ...
    const sB=$("btnEditSaveBottom"),cB=$("btnEditCloseBottom"); if (sB) sB.onclick=(e)=>{ e.preventDefault(); saveEditModal(); }; if (cB) cB.onclick=()=>closeModal(UI.editModal);
  }
...
// Wire action buttons
    ...
    tbody.querySelectorAll("[data-act='view-pdf']").forEach(btn=>{
      ...
      tbody.querySelectorAll("[data-act='resend']").forEach(btn=>{
 ...
 if (!leadId||!email) return;
          ...
          const token=STATE.session?.access_token||"";
          if (!token) { showNotice("Nicht eingeloggt.","err"); btn.disabled=false; btn.textContent="Erneut senden"; return; }
          try {
...
if (result.ok) { showNotice("E-Mail erneut gesendet an "+email,"ok"); try { localStorage.setItem("ab_photo_reminder_"+leadId,String(Date.now())); } catch {} }
  ...
  } catch(err) { console.error("loadPortalList error:",err); if(loading)loading.style.display="none"; if(empty){empty.style.display="block";empty.textContent="Fehler beim Laden.";} }
  }
  ...
  function bind() {
    ...
    if (UI.btnEditSaveBottom) UI.btnEditSaveBottom.addEventListener("click",(e)=>{ e.preventDefault(); saveEditModal(); });
...
/* ============================================================
   AnfrageBox Admin — Mobile Drawer & Filter Patch v2
   Loaded AFTER admin_pro_clean26.js
   Detects mobile (<768px) and restructures drawer + filters
   ============================================================ */
(function() {
  'use strict';
  ...
  function initFilterCollapse() {
    ...
    if (!ltLeft || !ltMid) return;

    // Don't add twice
    if (ltLeft.querySelector('.mob-filter-toggle')) return;

    var btn = document.createElement('button');
    btn.className = 'mob-filter-toggle';
    btn.type = 'button';
    btn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M3 4h18M7 9h10M10 14h4"/></svg> Filter';
    ltLeft.appendChild(btn);

    btn.addEventListener('click', function() {
      ltMid.classList.toggle('mob-filters-open');
      btn.innerHTML = ltMid.classList.contains('mob-filters-open')
        ? '<svg viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"/></svg> Schließen'
        : '<svg viewBox="0 0 24 24"><path d="M3 4h18M7 9h10M10 14h4"/></svg> Filter';
    });
  }

  /* ── SCREEN 2: Mobile drawer HTML generator ── */
  ...
  function getZipFromAddr(addr) {
    if (!addr || typeof addr !== 'object') return '';
    return String(addr.zip || addr.postal || addr.postcode || '').trim();
  }

  function buildMobileDrawer(row, helpers) {
    var p = helpers.getPayload(row);
    var name = helpers.customerName(row) || 'Ohne Name';
    var phone = helpers.customerPhone(row);
    var email = helpers.customerEmail(row);
    var fromA = helpers.getAddr(row, 'from');
    var toA = helpers.getAddr(row, 'to');
    var st = helpers.normalizeStatus(row.status);
    var fromZip = getZipFromAddr(fromA);
    var toZip = getZipFromAddr(toA);
    var routeShort = (fromZip && toZip) ? fromZip + ' > ' + toZip : helpers.addrLine(fromA) || '';
    var digits = helpers.normalizePhoneDigits(phone);
    var waPhone = digits || '';
    var offer = helpers.getOffer(row);
    ...
    // Inventory
    var rooms = helpers.getSelectedRooms(row);
    var items = helpers.getRoomItems(row);
    var qty = helpers.getRoomQty(row);
    var asm = helpers.getAssembly(row);
    var roomSet = new Set();
    ...
    roomSet.forEach(function(room) {
      var list = Array.isArray(items && items[room]) ? items[room] : [];
      if (!list.length) return;
      invHtml += '<div class="invRoom"><div class="invRoomTitle">' + esc(room) + '</div>';
      list.forEach(function(it) {
        var q = Math.max(1, Number(qty && qty[room] && qty[room][it] || 1));
        var sv = String(asm && asm[room] && asm[room][it] || '');
        var mark = helpers.svcMark ? helpers.svcMark(sv) : '';
        invHtml += '<div class="invLine"><div class="invQty">' + q + 'x</div><div class="invName">' + esc(it);
        if (mark) invHtml += ' <span style="color:#6b7280;font-size:11px;">' + esc(mark) + '</span>';
        invHtml += '</div></div>';
      });
      invHtml += '</div>';
    });
    if (!invHtml) invHtml = '<div style="color:#9ca3af;">Kein Inventar.</div>';

    // Cartons
    var cartons = helpers.cartonsData(row);
    var cartHtml = '';
    if (cartons.any) {
      if (cartons.small) cartHtml += '<div>Small: ' + esc(cartons.small) + '</div>';
      if (cartons.medium) cartHtml += '<div>Medium: ' + esc(cartons.medium) + '</div>';
      if (cartons.large) cartHtml += '<div>Large: ' + esc(cartons.large) + '</div>';
      if (cartons.notes) cartHtml += '<div style="margin-top:4px;color:#6b7280;">' + esc(cartons.notes) + '</div>';
    } else {
      cartHtml = '<div style="color:#9ca3af;">Keine Kartons.</div>';
    }

    // Route details
    var pickD = p.pickup_details || {};
    var dropD = p.dropoff_details || {};
    function addrDetail(label, addr, det) {
      return '<div style="font-weight:800;margin-bottom:4px;">' + esc(label) + '</div>' +
        '<div>' + esc(helpers.addrLine(addr) || '-') + '</div>' +
        '<div style="font-size:11px;color:#6b7280;margin-top:4px;">' +
        [det.housingType, det.floor ? 'Etage ' + det.floor : '', det.elevator, det.areaM2 ? det.areaM2 + ' m²' : '', det.parkingType]
          .filter(Boolean).map(esc).join(' / ') +
        '</div>' +
        (det.notes ? '<div style="font-size:11px;color:#6b7280;">' + esc(det.notes) + '</div>' : '');
    }
    var routeHtml = addrDetail('Beladestelle', fromA, pickD) +
      '<div style="height:10px;"></div>' +
      addrDetail('Entladestelle', toA, dropD);

    // Move info
    var move = helpers.formatDate ? helpers.formatDate(row.move_date || p.move_date || '') : (row.move_date || p.move_date || '');
    var timeWindow = p.move_time_window || '';

    // Build zones
    var html = '<div class="mob-drawer-wrapper" style="display:none;">';

    // ZONE 1: Sticky header
    html += '<div class="mob-drawer-header">';
    html += '<div class="mob-name">' + esc(name) + '</div>';
    html += '<div class="mob-meta">';
    html += '<span class="mob-status-pill ' + statusPillClass(st) + '">' + esc(st) + '</span>';
    if (routeShort) html += '<span class="mob-route-short">' + esc(routeShort) + '</span>';
    if (move) html += '<span class="mob-route-short">Umzug: ' + esc(move) + (timeWindow ? ' (' + esc(timeWindow) + ')' : '') + '</span>';
    html += '</div>';
    // Action buttons
    html += '<div class="mob-actions-row">';
    if (phone) {
      html += '<a class="mob-action-btn call-btn" href="tel:' + esc(phone) + '">';
      html += '<svg viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>';
      html += 'Anrufen</a>';
    } else {
      html += '<span class="mob-action-btn" style="opacity:.3;pointer-events:none;">Anrufen</span>';
    }
    if (waPhone) {
      html += '<a class="mob-action-btn wa-btn" href="https://api.whatsapp.com/send?phone=' + encodeURIComponent(waPhone) + '" target="_blank">';
      ...
      html += '<span class="mob-action-btn" style="opacity:.3;pointer-events:none;">WhatsApp</span>';
    }
    if (email) {
      html += '<a class="mob-action-btn email-btn" href="mailto:' + esc(email) + '">';
      html += '<svg viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 6L2 7"/></svg>';
      html += 'E-Mail</a>';
    } else {
      html += '<span class="mob-action-btn" style="opacity:.3;pointer-events:none;">E-Mail</span>';
    }
    html += '</div>'; // mob-actions-row
    html += '</div>'; // mob-drawer-header

    // ZONE 2: Quick actions
    html += '<div class="mob-drawer-quick">';

    // Status change
    html += '<div class="mob-quick-card">';
    html += '<div class="mob-qc-title">Status</div>';
    html += '<div class="mob-status-change">';
    html += '<select class="select" id="mob_k_status">' + statusOpts + '</select>';
    html += '<button class="btn green" id="mob_saveStatus" type="button">Speichern</button>';
    html += '</div></div>';

    // Angebot
    html += '<div class="mob-quick-card">';
    html += '<div class="mob-qc-title">Angebot</div>';
    html += '<div class="mob-offer-row">';
    html += '<input class="input" id="mob_offerPrice" placeholder="Preis EUR" value="' + esc(offer.price || '') + '" inputmode="decimal" />';
    html += '<button class="btn green" id="mob_btnOfferSend" type="button">Senden</button>';
    html += '</div></div>';

    // Portal status (loaded async)
    html += '<div class="mob-quick-card">';
    html += '<div class="mob-qc-title">Kundenportal</div>';
    html += '<div class="mob-portal-status" id="mob_portalStatus">';
    html += '<span class="mob-ps-label" style="color:#9ca3af;">Laden...</span>';
    html += '</div></div>';

    html += '</div>'; // mob-drawer-quick

    // ZONE 3: Collapsible details
    html += '<div class="mob-drawer-details">';

    // Route
    html += '<details class="mob-acc"><summary>Route</summary>';
    html += '<div class="mob-acc-body">' + routeHtml + '</div></details>';

    // Inventar
    html += '<details class="mob-acc"><summary>Inventar</summary>';
    html += '<div class="mob-acc-body">' + invHtml + '</div></details>';

    // Kartons
    html += '<details class="mob-acc"><summary>Kartons</summary>';
    html += '<div class="mob-acc-body">' + cartHtml + '</div></details>';

    // Fotos
    html += '<details class="mob-acc"><summary>Kundenfotos</summary>';
    html += '<div class="mob-acc-body" id="mob_leadPhotos"><span style="color:#9ca3af;">Laden...</span></div></details>';

    // Timeline
    html += '<details class="mob-acc"><summary>Aktivitäten</summary>';
    html += '<div class="mob-acc-body" id="mob_leadTimeline"><span style="color:#9ca3af;">Laden...</span></div></details>';

    // Delete button at bottom
    html += '<div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--line,#e5e7eb);">';
    html += '<button class="btn" id="mob_btnDelete" type="button" style="width:100%;color:#dc2626;border-color:#fecaca;font-weight:800;">Lead loschen</button>';
    html += '</div>';

    html += '</div>'; // mob-drawer-details
    html += '</div>'; // mob-drawer-wrapper

    return html;
  }

  /* ── Hook into renderDrawer ── */
  function patchRenderDrawer() {
    // We need access to the internal helpers. They're inside the IIFE of admin_pro_clean26.js.
    // Strategy: after renderDrawer runs (which sets drawerBody.innerHTML with desktop layout),
    // we append mobile HTML and bind events.

    // Wait for the global __AB object or for the drawer body to exist
    var drawerBody = document.getElementById('drawerBody');
    if (!drawerBody) return;

    // Use MutationObserver to detect when drawerBody innerHTML changes
  ...
  function onDrawerRendered() {
    var drawerBody = document.getElementById('drawerBody');
    if (!drawerBody) return;

    // Check if mobile wrapper already exists
    if (drawerBody.querySelector('.mob-drawer-wrapper')) return;

    // Extract data from the desktop layout that was just rendered
    var leadLayout = drawerBody.querySelector('.leadLayout');
    if (!leadLayout) return;

    // We need to read data from the rendered desktop HTML
    // Name from drawer title
    var drawerTitle = document.getElementById('drawerTitle');
    var name = drawerTitle ? drawerTitle.textContent : 'Lead';

    // Status from the select
    var kStatus = document.getElementById('k_status');
    var currentStatus = kStatus ? kStatus.value : 'Neu';

    // Route from route boxes
    var routeBoxes = leadLayout.querySelectorAll('.routeBox');
    var fromText = routeBoxes[0] ? routeBoxes[0].textContent.trim() : '';
    var toText = routeBoxes[1] ? routeBoxes[1].textContent.trim() : '';

    // Extract zip codes from route text
    function extractZip(text) {
      var match = text.match(/\b(\d{4,5})\b/);
      return match ? match[1] : '';
    }
    var fromZip = extractZip(fromText);
    var toZip = extractZip(toText);
    var routeShort = (fromZip && toZip) ? fromZip + ' > ' + toZip : '';
    if (!routeShort && fromText) routeShort = fromText.substring(0, 30);

    // Contact info from kontakt card
    var kontaktCard = leadLayout.querySelector('.leadSide');
    var phoneInput = kontaktCard ? kontaktCard.querySelectorAll('.input')[1] : null; // 2nd input = Telefon
    var emailInput = kontaktCard ? kontaktCard.querySelectorAll('.input')[2] : null; // 3rd input = E-Mail
    ...
    var offerPrice = offerPriceInput ? offerPriceInput.value : '';

    // Move date from Termin input
    var terminInputs = kontaktCard ? kontaktCard.querySelectorAll('.miniCard') : [];
    var moveDate = '';
    var timeWindow = '';
    // Try to find from the status card
    if (terminInputs[0]) {
      var fields = terminInputs[0].querySelectorAll('.input');
      if (fields[0]) moveDate = fields[0].value; // Termin
    }

    // Phone digits for WhatsApp
    var digits = String(phone).replace(/[^\d+]/g, '').replace(/^\+/, '');

    // Status options
    var statusList = ['Neu', 'In Bearbeitung', 'Angebot gesendet', 'Gebucht', 'Abgeschlossen', 'Archiviert'];
    var statusOpts = statusList.map(function(x) {
      return '<option ' + (x === currentStatus ? 'selected' : '') + '>' + esc(x) + '</option>';
    }).join('');

    // Extract inventory HTML from desktop accordions
    var desktopAccs = leadLayout.querySelectorAll('.acc, details');
    var invHtml = '', cartHtml = '', routeHtml = '';
    desktopAccs.forEach(function(acc) {
      var summaryEl = acc.querySelector('summary, .accHead, .accTitle');
      var bodyEl = acc.querySelector('.accBody, div:not(summary)');
      if (!summaryEl) return;
      var title = summaryEl.textContent.trim().toLowerCase();
      var bodyContent = bodyEl ? bodyEl.innerHTML : '';
      if (title.includes('inventar') || title.includes('möbel')) invHtml = bodyContent;
      else if (title.includes('karton')) cartHtml = bodyContent;
    });

    // Build mobile HTML
    var html = '<div class="mob-drawer-wrapper">';

    // ZONE 1: Sticky header
    html += '<div class="mob-drawer-header">';
    html += '<div class="mob-name">' + esc(name) + '</div>';
    html += '<div class="mob-meta">';
    html += '<span class="mob-status-pill ' + statusPillClass(currentStatus) + '">' + esc(currentStatus) + '</span>';
    if (routeShort) html += '<span class="mob-route-short">' + esc(routeShort) + '</span>';
    if (moveDate) html += '<span class="mob-route-short">' + esc(moveDate) + '</span>';
    html += '</div>';
    ...
    if (digits) {
      html += '<a class="mob-action-btn wa-btn" href="https://api.whatsapp.com/send?phone=' + encodeURIComponent(digits) + '" target="_blank">';
      html += '<svg viewBox="0 0 24 24"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>';
      html += 'WhatsApp</a>';
    } else {
      html += '<span class="mob-action-btn" style="opacity:.3;pointer-events:none;">WhatsApp</span>';
    ...
    // Status
    html += '<div class="mob-quick-card"><div class="mob-qc-title">Status</div>';
    html += '<div class="mob-status-change">';
    html += '<select class="select" id="mob_k_status">' + statusOpts + '</select>';
    html += '<button class="btn green" id="mob_saveStatus" type="button">Speichern</button>';
    html += '</div></div>';
    ...
    // Angebot
    html += '<div class="mob-quick-card"><div class="mob-qc-title">Angebot</div>';
    html += '<div class="mob-offer-row">';
    html += '<input class="input" id="mob_offerPrice" placeholder="Preis EUR" value="' + esc(offerPrice) + '" inputmode="decimal"/>';
    html += '<button class="btn green" id="mob_btnOfferSend" type="button">Senden</button>';
    html += '</div></div>';

    // Portal
    html += '<div class="mob-quick-card"><div class="mob-qc-title">Kundenportal</div>';
    html += '<div class="mob-portal-status" id="mob_portalStatus">';
    html += '<span class="mob-ps-label" style="color:#9ca3af;">Laden...</span>';
    html += '</div></div>';
    ...
    // ZONE 3: Details (accordions, closed by default)
    html += '<div class="mob-drawer-details">';

    // Route
    html += '<details class="mob-acc"><summary>Route</summary>';
    html += '<div class="mob-acc-body">';
    html += '<div style="font-weight:800;margin-bottom:4px;">Beladestelle</div>';
    html += '<div>' + esc(fromText || '-') + '</div>';
    html += '<div style="height:10px;"></div>';
    html += '<div style="font-weight:800;margin-bottom:4px;">Entladestelle</div>';
    html += '<div>' + esc(toText || '-') + '</div>';
    html += '</div></details>';

    // Inventar
    html += '<details class="mob-acc"><summary>Inventar</summary>';
    html += '<div class="mob-acc-body">' + (invHtml || '<span style="color:#9ca3af;">Kein Inventar.</span>') + '</div></details>';

    // Kartons
    html += '<details class="mob-acc"><summary>Kartons</summary>';
    html += '<div class="mob-acc-body">' + (cartHtml || '<span style="color:#9ca3af;">Keine Kartons.</span>') + '</div></details>';
    ...
    html += '<details class="mob-acc"><summary>Aktivitaten</summary>';
    html += '<div class="mob-acc-body" id="mob_leadTimeline"><span style="color:#9ca3af;">Laden...</span></div></details>';
    ...
    // Append mobile HTML to drawer body
    drawerBody.insertAdjacentHTML('beforeend', html);

    // Bind mobile events
    bindMobileDrawerEvents();

    // Sync portal status from desktop portal card
    syncPortalToMobile();

    // Sync photos and timeline from desktop
    syncAsyncContent();
  }

  function bindMobileDrawerEvents() {
    // Status save — trigger the desktop status change
    var mobStatus = document.getElementById('mob_k_status');
    var mobSaveBtn = document.getElementById('mob_saveStatus');
    if (mobStatus && mobSaveBtn) {
      mobSaveBtn.addEventListener('click', function() {
        var desktopStatus = document.getElementById('k_status');
        if (desktopStatus) {
          desktopStatus.value = mobStatus.value;
          desktopStatus.dispatchEvent(new Event('change'));
        }
      });
    }

    // Offer send — sync price to desktop, then trigger desktop send
    var mobPrice = document.getElementById('mob_offerPrice');
    var mobOfferBtn = document.getElementById('mob_btnOfferSend');
    if (mobPrice && mobOfferBtn) {
      mobOfferBtn.addEventListener('click', function() {
        var desktopPrice = document.getElementById('offerPrice');
        if (desktopPrice) desktopPrice.value = mobPrice.value;
        var desktopSend = document.getElementById('btnOfferSend');
        if (desktopSend) desktopSend.click();
      });
    }

    // Delete — trigger desktop delete logic
    var mobDelete = document.getElementById('mob_btnDelete');
    if (mobDelete) {
      mobDelete.addEventListener('click', function() {
        // Find the desktop drawer delete or use the drawerHead buttons
        // The desktop has delete in the "more" menu or drawer buttons.
        // We trigger a confirm + delete via the existing mechanism
        if (!confirm('Lead wirklich loschen?')) return;
        // Find delete button from desktop drawer or trigger via global
        var desktopDelete = document.querySelector('.dHeadRight .btn[data-act="delete"]');
        if (desktopDelete) {
          desktopDelete.click();
          return;
        }
        // Fallback: try to find any delete mechanism
        // Look for the "Loschen" text button in drawer
        var allBtns = document.querySelectorAll('.drawerBody .btn, .drawerHead .btn');
        for (var i = 0; i < allBtns.length; i++) {
          if (allBtns[i].textContent.trim().toLowerCase().includes('löschen') ||
              allBtns[i].textContent.trim().toLowerCase().includes('loschen') ||
              allBtns[i].dataset.act === 'delete') {
            allBtns[i].click();
            return;
          }
        }
  ...
  function syncPortalToMobile() {
    // Watch the desktop portal card and mirror status to mobile
    var desktopPortal = document.getElementById('portalCardStatus');
    var mobPortal = document.getElementById('mob_portalStatus');
    if (!desktopPortal || !mobPortal) return;

    var portalObserver = new MutationObserver(function() {
      // Read desktop portal status
      var statusSpan = desktopPortal.querySelector('span[style*="font-weight:700"]');
      var statusDot = desktopPortal.querySelector('span[style*="border-radius:50%"]');
      var dotColor = statusDot ? statusDot.style.background : '#6b7280';
      var statusText = statusSpan ? statusSpan.textContent : 'Laden...';

      // Check for signed PDF or action buttons
      var actEl = document.getElementById('portalCardActions');
      var signedLink = actEl ? actEl.querySelector('a.btn.green') : null;
      var remBtn = actEl ? actEl.querySelector('#btnPortalRemEmail, a[href*="whatsapp"]') : null;

      var mobHtml = '<span class="mob-ps-label"><span class="mob-ps-dot" style="background:' + esc(dotColor) + ';"></span>' + esc(statusText) + '</span>';

      if (signedLink) {
        mobHtml += '<a class="btn green" href="' + esc(signedLink.href) + '" target="_blank" style="height:34px;padding:0 12px;font-size:11px;font-weight:800;text-decoration:none;">Signiertes PDF</a>';
      } else if (remBtn && statusText.includes('Offen')) {
        mobHtml += '<button class="btn soft" id="mob_photoReminder" type="button" style="height:34px;padding:0 12px;font-size:11px;font-weight:800;">Foto-Erinnerung</button>';
      }

      mobPortal.innerHTML = mobHtml;

      // Bind photo reminder
      var mobReminder = document.getElementById('mob_photoReminder');
      if (mobReminder) {
        mobReminder.addEventListener('click', function() {
          var desktopRemBtn = document.getElementById('btnPortalRemEmail');
          if (desktopRemBtn) desktopRemBtn.click();
          else {
            // Try WhatsApp link
            var waLink = actEl ? actEl.querySelector('a[href*="whatsapp"]') : null;
            if (waLink) window.open(waLink.href, '_blank');
          }
        });
      }
    });
    portalObserver.observe(desktopPortal, { childList: true, subtree: true, characterData: true });

    // Also observe the actions container
    var actEl = document.getElementById('portalCardActions');
    if (actEl) {
      portalObserver.observe(actEl, { childList: true, subtree: true });
    }
  }

  function syncAsyncContent() {
    // Photos: watch desktop #leadPhotos and mirror to mobile #mob_leadPhotos
    watchAndMirror('leadPhotos', 'mob_leadPhotos');
    // Timeline: watch desktop #leadTimeline and mirror to mobile #mob_leadTimeline
    watchAndMirror('leadTimeline', 'mob_leadTimeline');
  }

  function watchAndMirror(sourceId, targetId) {
    ...
    // Also copy current content if already loaded
    if (source.innerHTML && !source.innerHTML.includes('Laden')) {
      target.innerHTML = source.innerHTML;
    }
  }
  ...
  if (document.readyState === 'loading') {
    ...
    // Small delay to ensure admin_pro_clean26.js has initialized
    setTimeout(init, 100);
  }
