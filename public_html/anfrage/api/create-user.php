...
// Auth
$provided = $_GET['token'] ?? '';
if ($provided !== $SECRET_TOKEN) { http_response_code(403); echo "Forbidden. Bad token."; exit; }

// Load service role
$secrets = [];
$sp = __DIR__ . '/_secrets.php';
if (is_readable($sp)) { $tmp = require $sp; if (is_array($tmp)) $secrets = $tmp; }
$SERVICE_ROLE = (string)($secrets['SUPABASE_SERVICE_ROLE'] ?? '');
if ($SERVICE_ROLE === '') { echo "ERROR: SUPABASE_SERVICE_ROLE missing in _secrets.php"; exit; }
